...
class GitLabExplorer:
    ...
    def get_mrs_for_branch(
        self, branch_name: str, state: str = "opened"
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for mr in mrs:
                ...
                if hasattr(mr, "head_pipeline") and mr.head_pipeline:
                    mr_data["pipeline_status"] = mr.head_pipeline.get(
                        "status", "unknown"
                    )
    ...
    def get_pipeline_details(
        self, pipeline_id: int, use_cache: bool = True, verbose: bool = False
        ...
        if use_cache:
            ...
            if cached:
                if verbose:
                    print(
                        f"✓ Retrieved pipeline {pipeline_id} from cache ({time.time() - start:.3f}s)"
                    )
                return cached
            elif verbose:
                print(
                    f"✗ Pipeline {pipeline_id} not in cache ({time.time() - start:.3f}s)"
                )
        ...
        try:
            ...
            if pipeline.attributes.get("status", "").lower() in COMPLETE_STATUSES:
                ...
                if verbose:
                    print(
                        f"✓ Cached completed pipeline {pipeline_id} (status: {pipeline.attributes.get('status')})"
                    )
            elif verbose:
                print(
                    f"✗ Not caching pipeline {pipeline_id} (status: {pipeline.attributes.get('status')} - still running)"
                )
    ...
    def get_pipeline_from_cache(
        self, pipeline_id: int, verbose: bool = False
    ) -> Optional[Dict[str, Any]]:
        conn = sqlite3.connect(self.db_file)
        cur = conn.cursor()
        ...
        if verbose:
            ...
            cur.execute(
                "SELECT pipeline_id, created_at FROM pipelines ORDER BY created_at DESC LIMIT 5"
            )
            recent = cur.fetchall()
            if recent:
    ...
    def get_job_status_summary(
        ...
        summary["progress"] = {
            ...
            "total": summary["total"],
            "percentage": (
                int((completed / summary["total"] * 100)) if summary["total"] > 0 else 0
            ),
        }
    ...
    def extract_failures_from_trace(
        self, trace: str, job_name: str = ""
    ) -> Dict[str, Any]:
        ...
        if "pylint" in job_name_lower:
            ...
            test_word in job_name_lower
            for test_word in ["test", "pytest", "integration", "unit"]
        ):
    ...
    def extract_pylint_failures(self, trace: str, failures: Dict) -> Dict[str, Any]:
        ...
        if violations:
            failures["short_summary"] = (
                f"Pylint found {len(violations)} violation(s):\n"
                + "\n".join(violations[:20])
            )
        ...
        exit_pattern = re.compile(
            r"ERROR: Job failed: command terminated with exit code (\d+)"
        )
        exit_match = exit_pattern.search(trace)
        if exit_match and not violations:
            failures["short_summary"] = (
                f"Pylint failed with exit code {exit_match.group(1)}"
            )
    ...
    def extract_linting_failures(self, trace: str, failures: Dict) -> Dict[str, Any]:
        ...
        ruff_pattern = re.compile(
            r"^(.+\.py):(\d+):(\d+):\s+([A-Z]\d+)\s+(.+)$", re.MULTILINE
        )
        ...
        if lint_errors:
            failures["short_summary"] = (
                f"Linting found {len(lint_errors)} issue(s):\n"
                + "\n".join(lint_errors[:15])
            )
        ...
        if failed_index > 0:
            ...
            for line in lines:
                if any(
                    ...
                    for keyword in ["error", "failed", "exception", "fatal", "abort"]
                ):
...
class PipelineCLI:
    ...
    def cmd_branch_mrs(self, args):
        ...
        print("-" * 120)
        print(
            f"{'MR':<8} {'Status':<10} {'Pipeline':<12} {'Title':<50} {'Author':<15} {'Target'}"
        )
        print("-" * 120)
        ...
        for mr in mrs:
            ...
            print(
                f"!{mr['iid']:<7} {state_display} {pipeline_display:<12} {title:<50} {mr['author']:<15} {mr['target_branch']}"
            )
    ...
    def cmd_mr_info(self, args):
        ...
        try:
            ...
            if hasattr(mr, "head_pipeline") and mr.head_pipeline:
                ...
                print(
                    f"Pipeline:      {pipeline_display} (ID: {mr.head_pipeline.get('id')})"
                )
            ...
            if hasattr(mr, "approvals_required"):
                ...
                approved_by = (
                    [u["user"]["username"] for u in approvals.approved_by]
                    if hasattr(approvals, "approved_by")
                    else []
                )
                approval_status = f"{len(approved_by)}/{approvals.approvals_required}"
                if len(approved_by) >= approvals.approvals_required:
                    print(
                        f"Approvals:     \033[92m✅ {approval_status}\033[0m - {', '.join(approved_by)}"
                    )
                else:
                    print(
                        f"Approvals:     ⏸ {approval_status}"
                        + (f" - {', '.join(approved_by)}" if approved_by else "")
                    )
    ...
    def cmd_mr_pipelines(self, args):
        ...
        for p in pipelines[:10]:  # Limit to 10 most recent
            ...
            print(
                f"{p['id']:<10} {status_display} {p['ref'][:30]:<30} {p['sha']:<10} {created_str}"
            )
    ...
    def cmd_pipeline_status(self, args):
        ...
        summary = self.explorer.get_job_status_summary(
            args.pipeline_id, verbose=self.verbose
        )
        ...
        print(
            f"Progress: [{bar}] {progress['completed']}/{progress['total']} ({progress['percentage']}%)"
        )
        ...
        if args.detailed:
            ...
            for stage in stage_order:
                ...
                for job in sorted(jobs, key=lambda x: x["name"]):
                    ...
                    duration = (
                        self.explorer.format_duration(job["duration"])
                        if job["duration"]
                        else "-"
                    )
                    ...
                    if status_color:
                        print(
                            f"  {job_icon} {status_color}{job['id']:<10} {job_name:<50} {duration:<10}\033[0m"
                        )
                    else:
                        print(
                            f"  {job_icon} {job['id']:<10} {job_name:<50} {duration:<10}"
                        )
        else:
            ...
            if summary["failed_jobs"]:
                ...
                for job in summary["failed_jobs"]:
                    ...
                    print(
                        f"{job['id']:<12} {job['stage']:<15} {job['name'][:40]:<40} {duration}"
                    )

    def cmd_pipeline_jobs(self, args):
        """List all jobs in a pipeline with optional filtering."""
        data = self.explorer.get_pipeline_details(
            args.pipeline_id, verbose=self.verbose
        )
        ...
        if hasattr(args, "status") and args.status:
            jobs = [
                j for j in jobs if j.get("status", "").lower() == args.status.lower()
            ]
        ...
        if output_format == "json":
            ...
            print(
                f"{'ID':<12} {'Status':<10} {'Stage':<15} {'Name':<40} {'Duration':<10}"
            )
            print("-" * 87)
            for job in jobs:
                duration = self.explorer.format_duration(job.get("duration"))
                print(
                    f"{job['id']:<12} {job['status']:<10} {job.get('stage', ''):<15} {job['name'][:40]:<40} {duration:<10}"
                )
            ...
            print("-" * 100)
            print(
                f"{'ID':<12} {'Status':<10} {'Stage':<15} {'Name':<40} {'Duration':<10} {'Finished'}"
            )
            print("-" * 100)
            ...
            for job in jobs:
                ...
                print(
                    f"{job['id']:<12} {status_display} {job.get('stage', ''):<15} {job['name'][:40]:<40} {duration:<10} {finished}"
                )
    ...
    def cmd_batch_failures(self, args):
        ...
        for job_id in args.job_ids:
            ...
            print(
                f"Status: {details['status']} | Duration: {self.explorer.format_duration(details['duration'])}"
            )
...
Examples:
  ...
  %(prog)s status <pipeline_id_from_above>
        ...
        branch_parser = subparsers.add_parser(
            "branch", help="Get merge requests for a branch"
        )
        branch_parser.add_argument(
            "branch_name",
            help="Branch name (use $(git branch --show-current) for current)",
        )
        branch_parser.add_argument(
            "--state",
        ...
        branch_parser.add_argument(
            "--latest", action="store_true", help="Show latest MR and pipeline IDs"
        )

        # MR pipelines command
        mr_parser = subparsers.add_parser(
            "mr", help="Get pipelines for a merge request"
        )
        mr_parser.add_argument("mr_id", type=int, help="Merge request ID")
        mr_parser.add_argument(
            "--latest", action="store_true", help="Show latest pipeline ID"
        )

        # MR info command
        mr_info_parser = subparsers.add_parser(
            "mr-info", help="Get detailed information about a merge request"
        )
        mr_info_parser.add_argument("mr_id", type=int, help="Merge request ID")
        mr_info_parser.add_argument(
            "--pipelines", "-p", action="store_true", help="Also show recent pipelines"
        )

        # Pipeline status command
        status_parser = subparsers.add_parser(
            "status", help="Get job status summary for a pipeline"
        )
        ...
        jobs_parser.add_argument(
            "--status", help="Filter by status (success, failed, running, etc.)"
        )
        ...
        failures_parser = subparsers.add_parser(
            "failures", help="Get failure details for a job"
        )
        ...
        batch_parser = subparsers.add_parser(
            "batch-failures", help="Get failures for multiple jobs"
        )
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "config":
        ...
        parser.add_argument(
            "--project", help="GitLab project path (e.g., group/project)"
        )
        parser.add_argument(
