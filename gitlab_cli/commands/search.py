...
class SearchCommand(BaseCommand):
    """Handle search and filtering for pipelines and MRs"""
        ...
        try:
            ...
            if hasattr(args, 'since') and args.since:
                try:
                    ...
                    pipelines = [
                        ...
                        if datetime.fromisoformat(p.created_at.replace('Z', '+00:00')) >= since_date
                    ]
            ...
            if hasattr(args, 'before') and args.before:
                try:
                    ...
                    pipelines = [
                        p for p in pipelines 
                        if datetime.fromisoformat(p.created_at.replace('Z', '+00:00')) <= before_date
                    ]
                except ValueError as e:
                    print(f"Error: {e}")
                    return
            ...
            if output_format == "json":
                ...
                print("=" * 120)
                print(f"{'ID':<10} {'Status':<10} {'Source':<10} {'Branch/Tag':<25} {'User':<15} {'Created':<20}")
                print("-" * 120)
                
                for p in pipelines:
                    ref_display = p.ref[:22] + "..." if len(p.ref) > 25 else p.ref
                    user_display = p.user.get('username', 'N/A')[:15] if hasattr(p, 'user') and p.user else 'N/A'
    ...
    def search_mrs(self, cli, args, output_format):
        ...
        try:
            ...
            if output_format == "json":
                    ...
                    "merge_requests": [
                        {
                            ...
                            "web_url": mr.web_url,
                            "draft": mr.draft if hasattr(mr, 'draft') else mr.work_in_progress
                        }
                        for mr in mrs
                    ]
                }
                ...
                print("=" * 120)
                print(f"{'MR':<8} {'State':<10} {'Author':<15} {'Title':<40} {'Source → Target':<30}")
                print("-" * 120)
                
                for mr in mrs:
                    title_display = mr.title[:37] + "..." if len(mr.title) > 40 else mr.title
                ...
                for mr in mrs:
                    ...
                    draft_str = " 📝 DRAFT" if (hasattr(mr, 'draft') and mr.draft) or (hasattr(mr, 'work_in_progress') and mr.work_in_progress) else ""
