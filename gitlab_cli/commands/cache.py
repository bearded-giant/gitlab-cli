...
class CacheCommand(BaseCommand):
    ...
    def add_arguments(self, subparsers):
        ...
        cache_subparsers = parser.add_subparsers(
            dest="cache_action", help="Cache management actions"
        )
        ...
        clear_parser.add_argument(
            "--all", action="store_true", help="Clear all cache (requires confirmation)"
        )
        clear_parser.add_argument(
            "--pipeline", type=int, help="Clear specific pipeline from cache"
        )
        clear_parser.add_argument(
            "--older-than", type=int, help="Clear pipelines older than N days"
        )
        clear_parser.add_argument(
            "--force", action="store_true", help="Skip confirmation prompts"
        )

        list_parser = cache_subparsers.add_parser("list", help="List cached items")
        ...
        info_parser = cache_subparsers.add_parser(
            "info", help="Show cache configuration and location"
        )
    ...
    def clear_cache(self, cache_file, args):
        ...
        if args.pipeline:
            ...
            conn.commit()
            print(
                f"Cleared pipeline {args.pipeline} from cache ({affected} entries removed)"
            )
            ...
            cur.execute(
                "SELECT COUNT(*) FROM pipelines WHERE created_at < ?", (cutoff_date,)
            )
            ...
            if not args.force:
                response = input(
                    f"Delete {count} pipelines older than {args.older_than} days? (y/N): "
                )
                if response.lower() != "y":
                    print("Cancelled")
                    return
    ...
    def list_cache(self, cache_file, args):
        ...
        print("=" * 80)
        print(
            f"{'Pipeline ID':<12} {'Status':<10} {'Branch/Tag':<20} {'Cached At':<20} {'Size':<10}"
        )
        print("-" * 80)
        ...
        for pid, created, size, status, ref in pipelines:
            ...
            print(
                f"{pid:<12} {status_display:<10} {ref_display:<20} {created_display:<20} {size_display:<10}"
            )
        cur.execute("SELECT COUNT(*) FROM pipelines")
    ...
    def show_info(self, config, cache_file):
        ...
        print("\nCache Behavior:")
        print(
            "  - Only completed pipelines are cached (success, failed, canceled, skipped)"
        )
